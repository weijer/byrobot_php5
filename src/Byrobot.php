...
final class Byrobot
{
    ...
    private function send($str, $param = [], $isArr = false)
    {
        ...
        $build = $this->build($str);
        $url = $build['url'];
        $method = $build['method'];
        $desc = $build['desc'];
        $res = $this->request($url, $method, $param, $isArr);
        $data = $res[0];
        $msg  = $res[1];
        if ($msg) {
     ...
     * @return array
     */
    protected function request($url, $method, $param = [], $isArr = false)
    {
        $msg = $json = null;
        try {
     ...
     * @return string
     */
    private function generateSign(string $date)
    {
